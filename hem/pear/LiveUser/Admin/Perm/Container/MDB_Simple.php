 ...
 * @version $Id: MDB_Simple.php,v 1.1 2004/06/08 11:42:53 mloitzl Exp $
 * @package LiveUser
 */
class LiveUser_Admin_Perm_Container_MDB_Simple extends LiveUser_Admin_Perm_Common
{
        ...
        $query = 'INSERT INTO
                ...
                VALUES
                  (
                    ...
                    ' . $this->dbc->getValue('text', $native_name) . '
                  )';
    ...
    function updateLanguage($language, $native_name, $language_name, $language_description = null)
    {
        ...
        $query = 'UPDATE
                  ' . $this->prefix . 'languages
                SET
                  native_name     = ' . $this->dbc->getValue('text', $native_name) . '
                WHERE
                  language_id     = ' . $this->dbc->getValue('integer', $this->_langs[$language]);
    ...
    function updateAuthUserId($permId, $authId, $authName = false)
    {
        ...
        $query = '
            ...
            SET';
        if ($authName !== false) {
            $query .= ' auth_container_name=' . $this->dbc->getValue('text', $authName) . ',';
        }
        $query .= '
                auth_user_id=' . $this->dbc->getValue('text', $authId) . '
            WHERE
