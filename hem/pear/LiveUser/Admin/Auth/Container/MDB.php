 ...
 * @version $Id: MDB.php,v 1.1 2004/06/08 11:42:52 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
class LiveUser_Admin_Auth_Container_MDB extends LiveUser_Admin_Auth_Common
{
        ...
        $owner_group_id = null, $authId = null, $customFields = array())
    {
        ...
        if (isset($this->authTableCols['is_active'])) {
            ...
            $val[] = $this->dbc->getValue($this->authTableCols['is_active']['type'], $active);
        }

        if (isset($this->authTableCols['owner_user_id'])) {
            $col[] = $this->authTableCols['owner_user_id']['name'];
            $val[] = $this->dbc->getValue($this->authTableCols['owner_user_id']['type'], $owner_user_id);
        }
        ...
        $owner_user_id = null, $owner_group_id = null, $customFields = array())
    {
        ...
        if (isset($owner_user_id)) {
            $updateValues[] =
                $this->authTableCols['owner_user_id'] . ' = ' . $this->dbc->getValue($this->authTableCols['owner_user_id']['type'], $owner_user_id);
        }
        ...
        if (sizeof($customFields) > 0) {
            foreach ($customFields as $k => $v) {
                $updateValues[] =
                    $v['name'] . ' = ' .
                        $this->dbc->getValue($v['type'], $v['value']);
            }
        }
    ...
    function getUsers($filters = array(), $addCustomFields = array(), $order = null, $rekey = false)
    {
        ...
        if (sizeof($filters) > 0) {
            ...
            foreach ($filters as $f => $v) {
                if (is_array($v)) {
                    $cond = ' ' . $v['cond'];
                    $where .= " $f" . $v['op'] . $this->dbc->getValue($v['type'], $v['value']) . $cond;
                } else {
                    $cond = ' AND';
                    $where .= " $f=$v" . $cond;
                }
