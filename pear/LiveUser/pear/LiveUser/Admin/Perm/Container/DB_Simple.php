 ...
 * @version $Id: DB_Simple.php,v 1.1 2004/07/16 13:59:01 mloitzl Exp $
 * @package LiveUser
 */
class LiveUser_Admin_Perm_Container_DB_Simple extends LiveUser_Admin_Perm_Common
{
    ...
    */
    ...
    function updateUser($permId, $authId = false, $authName = false, $type = false)
    {
        ...
        $update = array();
        if ($authId !== false) {
            $update[] = ' auth_user_id=' . $this->dbc->quoteSmart($authId);
        }
        if ($authName !== false) {
            $update[] = ' auth_container_name=' . $this->dbc->quoteSmart($authName);
        }
        if ($type !== false) {
            $update[] = ' perm_type=' . $this->dbc->quoteSmart($type);
        }
