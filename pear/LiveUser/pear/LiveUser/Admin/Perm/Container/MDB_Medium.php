 ...
 * @version $Id: MDB_Medium.php,v 1.1 2004/07/16 13:59:03 mloitzl Exp $
 * @package LiveUser
 */
class LiveUser_Admin_Perm_Container_MDB_Medium extends LiveUser_Admin_Perm_Container_MDB_Simple
{
        ...
        $define_name = null, $group_type = LIVEUSER_GROUP_TYPE_ALL, $customFields = array())
    {
        ...
        if (sizeof($customFields) > 0) {
            foreach ($customFields as $k => $v) {
                $col[] = $k;
                $val[] = $this->dbc->getValue($v['type'], $v['value']);
            }
        }
        ...
        $query = 'INSERT INTO
                ...
                VALUES
                  (
                    ...
                    ' . $this->dbc->getValue('integer', $group_type) . ',
        ...
        $active = null, $define_name = null, $group_type = null, $customFields = array())
    {
        $update = false;
        if(!is_null($active)) {
            $updateValues[] = 'is_active      = ' . $this->dbc->getValue('boolean', $active);
        }
        if (!is_null($define_name)) {
            $updateValues[] = 'define_name     = ' . $this->dbc->getValue('text', $define_name);
        }
        if (!is_null($group_type)) {
            $updateValues[] = 'group_type     = ' . $this->dbc->getValue('integer', $group_type);
        }
        if (sizeof($customFields) > 0) {
            foreach ($customFields as $k => $v) {
                $updateValues[] =
                    $k . ' = ' . $this->dbc->getValue($v['type'], $v['value']);
            }
        }
        if ($update) {
            $query = 'UPDATE
    ...
    function getGroups($options = null)
    {
        ...
        if (isset($options['where_group_type'])
            ...
            $query .= ' groups.group_type = ' . $this->dbc->getValue('integer', $options['where_group_type']) . ' AND';
        }
