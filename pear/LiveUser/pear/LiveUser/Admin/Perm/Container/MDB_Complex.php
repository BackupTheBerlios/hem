...
/**
 * MDB_Complex permission administration class
 *
 * @package  LiveUser
 ...
 */
require_once 'LiveUser/Admin/Perm/Container/MDB_Medium.php';
 ...
 *
 * Requirements:
 * - Files "common.php", "Container/MDB_Complex.php" in directory "Perm"
 ...
 * @version $Id: MDB_Complex.php,v 1.1 2004/07/16 13:59:02 mloitzl Exp $
 * @package LiveUser
 */
class LiveUser_Admin_Perm_Container_MDB_Complex extends LiveUser_Admin_Perm_Container_MDB_Medium
{
    /**
     * Constructor
     *
     ...
     * @return void
     */
    function LiveUser_Admin_Perm_Container_MDB_Complex(&$connectOptions)
    {
        $this->LiveUser_Admin_Perm_Container_MDB_Medium($connectOptions);
    }
    ...
    {
        $customFields['owner_user_id']  = array('value' => $owner_user,  'type' => 'integer');
        $customFields['owner_group_id'] = array('value' => $owner_group, 'type' => 'integer');
     ...
     * @return mixed boolean, MDB Error object or LiveUser Error Object
     */
    function assignSubgroup($group_id, $subgroup_id)
    {
        $query = 'SELECT subgroup_id FROM
                  ' . $this->prefix . 'group_subgroups
                  WHERE subgroup_id=' . $this->dbc->getValue('integer', $subgroup_id);
        ...
        $query = 'INSERT INTO
                ...
                VALUES
                  (
                    ...
                    ' . $this->dbc->getValue('integer', $subgroup_id) . '
                  )';
    ...
    function removeGroup($group_id, $recursive = false)
    {
        ...
        $query = 'DELETE FROM
                  ' . $this->prefix . 'group_subgroups
                WHERE
                  group_id = ' . $this->dbc->getValue('integer', $group_id) . ' OR
                  subgroup_id = ' . $this->dbc->getValue('integer', $group_id);
     ...
     * @return mixed integer or MDB Error object
     */
    ...
    function _updateImpliedStatus($right_id)
    {
        ...
        $query = 'UPDATE
                  ' . $this->prefix . 'rights
                SET
                  has_implied = ' . $this->dbc->getValue('boolean', ($count > 0)) . '
                WHERE
                  right_id = ' . $this->dbc->getValue('integer', $right_id);
        ...
        $query = 'INSERT INTO
                ...
                VALUES
                  (
                    ...
                    ' . $this->dbc->getValue('integer', $implied_right_id) . '
                  )';
    ...
    function unimplyRight($right_id, $implied_right_id)
    {
        ...
        $query = 'DELETE FROM
                  ' . $this->prefix . 'right_implied
                WHERE
                  right_id         = ' . $this->dbc->getValue('integer', $right_id) . ' AND
                  implied_right_id = ' . $this->dbc->getValue('integer', $implied_right_id);
        $result = $this->dbc->query($query);
     ...
     * @return mixed true on success or MDB error
     */
    ...
    function getGroups($options = null, $hierarchy = false)
    {
        ...
        if (isset($options['where_user_id'])
                && is_numeric($options['where_user_id'])) {
            $query .= ' groupusers.perm_user_id = ' . $this->dbc->getValue('integer', $options['where_user_id']) . ' AND
       ...
       $groups = $this->dbc->queryAll($query, $types, MDB_FETCHMODE_ASSOC);
        ...
        $subgroups = $this->dbc->queryAll($query, null, MDB_FETCHMODE_ASSOC);

        if (MDB::isError($subgroups)) {
            return $subgroups;
        }
    ...
    function getRights($options = null, $hierarchy = false)
    {
       ...
       $types = array('text', 'boolean', 'boolean', 'boolean', 'text', 'text');
       $rights = $this->dbc->queryAll($query, $types, MDB2_FETCHMODE_ASSOC);
        ...
        if ($options['with_inherited_rights'] &&
            ...
            if (MDB::isError($inherited_rights)) {
    ...
    function getImpliedRights($options = array())
    {
        ...
        $implied_rights = $this->dbc->queryAll($query, null, MDB_FETCHMODE_ASSOC);
    ...
    function getInheritedRights($options = array())
    {
        if (isset($options['where_user_id'])) {
            $query = 'SELECT
                     ...
                     WHERE groupusers.perm_user_id = ' . $this->dbc->getValue('integer', $options['where_user_id']);
        } else {
            $query = 'SELECT
                      subgroups.group_id as group_id
                     FROM
    ...
    function _updateLevelStatus($right_id)
    {
        ...
        $query = 'UPDATE
                  ' . $this->prefix . 'rights
                SET
                  has_level = ' . $this->dbc->getValue('boolean', ($count > 0)) . '
                WHERE
                  right_id = ' . $this->dbc->getValue('integer', $right_id);
        ...
        $query = 'INSERT INTO
                ...
                VALUES
                  (
                    ' . $permId . ',
                    ' . $this->dbc->getValue('integer', $right_id) . ',
                    ' . $this->dbc->getValue('integer', $right_level) . '
                  )';
