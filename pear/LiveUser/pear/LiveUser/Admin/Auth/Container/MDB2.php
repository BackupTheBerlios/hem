 ...
 * @version $Id: MDB2.php,v 1.1 2004/07/16 13:58:59 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
    ...
    function removeUser($authId)
    {
        ...
        $query = '
            DELETE FROM
                ' . $this->authTable . '
            WHERE
                auth_user_id = ' . $this->dbc->quote($authId, $this->authTableCols['required']['auth_user_id']['type']);
        ...
        if (isset($active)) {
            $updateValues[] =
                $this->authTableCols['optional']['is_active']['name'] . ' = '
                    . $this->dbc->quote($active, $this->authTableCols['optional']['is_active']['type']);
        }

        if (isset($owner_user_id)) {
            $updateValues[] =
                $this->authTableCols['optional']['owner_user_id'] . ' = '
                    . $this->dbc->quote($owner_user_id, $this->authTableCols['optional']['owner_user_id']['type']);
        }

        if (isset($owner_group_id)) {
            $updateValues[] =
                $this->authTableCols['optional']['owner_group_id'] . ' = '
                    . $this->dbc->quote($owner_group_id, $this->authTableCols['optional']['owner_group_id']['type']);
        }

        if (sizeof($this->authTableCols['custom']) > 0) {
            foreach ($this->authTableCols['custom'] as $alias => $field_data) {
                $updateValues[] = $field_data['name'] . '=' .
                    $this->dbc->quote($customFields[$alias], $field_data['type']);
            }
        }
