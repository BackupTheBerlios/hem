 ...
 * @version $Id: DB.php,v 1.1 2004/07/16 13:59:03 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
    ...
    function userExists($checkHandle = false, $checkPW = false)
    {
        ...
        if ($checkHandle !== false && $checkPW === false) {
            ...
            $sql    = 'SELECT ' . $this->authTableCols['required']['auth_user_id']['name'] . '
                       FROM   ' . $this->authTable . '
                       WHERE  ' . $this->authTableCols['required']['handle']['name'] . '=' . $this->dbc->quoteSmart($checkHandle);
        } elseif ($checkHandle === false && $checkPW !== false) {
            // only search for the first user with the given password
            $sql    = 'SELECT ' . $this->authTableCols['required']['auth_user_id']['name'] . '
                       FROM   ' . $this->authTable . '
                       WHERE  ' . $this->authTableCols['required']['passwd']['name'] . '=' . $this->dbc->quoteSmart($this->encryptPW($checkPW));
        } else {
            // check for a user with both handle and password matching
            $sql    = 'SELECT ' . $this->authTableCols['required']['auth_user_id']['name'] . '
                        FROM  ' . $this->authTable.'
                        WHERE ' . $this->authTableCols['required']['handle']['name'] . '=' . $this->dbc->quoteSmart($checkHandle) . '
                        AND   ' . $this->authTableCols['required']['passwd']['name'] . '=' . $this->dbc->quoteSmart($this->encryptPW($checkPW));
        }
