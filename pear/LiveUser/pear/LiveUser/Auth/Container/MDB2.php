 ...
 * @version $Id: MDB2.php,v 1.1 2004/07/16 13:59:04 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
class LiveUser_Auth_Container_MDB2 extends LiveUser_Auth_Common
{
    /**
     * dsn to connect to
     * 
    ...
    function _readUserData($handle, $passwd = false)
    {
        ...
        if ($passwd !== false) {
            ...
            $sql    = 'SELECT ' . implode(',', $fields) . '
                       FROM '   . $this->authTable . '
                       WHERE '  . $this->authTableCols['required']['handle']['name'] . '=' . $this->dbc->quote($handle, $this->authTableCols['handle']['type']).'
                        AND '   . $this->authTableCols['required']['passwd']['name'] . '=' . $this->dbc->quote($this->encryptPW($passwd), $this->authTableCols['passwd']['type']);
        } else {
    ...
    function userExists($checkHandle = false, $checkPW = false)
    {
        ...
        if ($checkHandle !== false && $checkPW === false) {
            ...
            $sql    = 'SELECT ' . $this->authTableCols['required']['auth_user_id']['name'] . '
                       FROM '   . $this->authTable . '
                       WHERE '  . $this->authTableCols['required']['handle']['name'] . '=' . $this->dbc->quote($checkHandle, $this->authTableCols['required']['handle']['type']);
        } elseif ($checkHandle === false && $checkPW !== false) {
            // only search for the first user with the given password
            $sql    = 'SELECT ' . $this->authTableCols['required']['auth_user_id']['name'] . '
                       FROM '   . $this->authTable . '
                       WHERE '  . $this->authTableCols['required']['handle']['name'] . '=' . $this->dbc->quote($this->encryptPW($checkPW), $this->authTableCols['required']['passwd']['type']);
        } else {
            // check for a user with both handle and password matching
            $sql    = 'SELECT ' . $this->authTableCols['required']['auth_user_id']['name'] . '
                        FROM '  . $this->authTable . '
                        WHERE ' . $this->authTableCols['required']['handle']['name'] . '=' . $this->dbc->quote($checkHandle, $this->authTableCols['required']['handle']['type']) . '
                        AND '   . $this->authTableCols['required']['passwd']['name'] . '=' . $this->dbc->quote($this->encryptPW($checkPW), $this->authTableCols['required']['passwd']['type']);
        }
