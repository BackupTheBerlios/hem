 ...
 * @version $Id: DB.php,v 1.1 2004/07/16 13:58:09 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
class LiveUser_Admin_Auth_Container_DB extends LiveUser_Admin_Auth_Common
{
        ...
        $owner_user_id = null, $owner_group_id = null, $customFields = array())
    {
        ...
        if (!empty($handle)) {
            $updateValues[] =
                $this->authTableCols['handle'] . ' = ' . $this->dbc->quoteSmart($handle);
        }
        ...
        if (isset($active)) {
            $active == true ? $active = 'Y' : $active = 'N';
            $updateValues[] =
                $this->authTableCols['is_active'] . ' = ' . $this->dbc->quoteSmart($active);
        }

        if (isset($owner_user_id)) {
            $updateValues[] =
                $this->authTableCols['owner_user_id'] . ' = ' . (int)$owner_user_id;
        }

        if (isset($owner_group_id)) {
            $updateValues[] =
                $this->authTableCols['owner_group_id'] . ' = ' . (int)$owner_group_id;
        }
        ...
        $query .= ' WHERE
            ' . $this->authTableCols['user_id'] . ' = ' . $this->dbc->quoteSmart($authId);
