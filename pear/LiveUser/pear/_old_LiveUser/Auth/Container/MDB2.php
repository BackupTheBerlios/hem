 ...
 * @version $Id: MDB2.php,v 1.1 2004/07/16 13:58:45 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
        ...
        if (isset($this->authTableCols['lastlogin'])) {
            $sql  = 'UPDATE ' . $this->authTable.'
                     SET '.$this->authTableCols['lastlogin']['name']
                        .'='.$this->dbc->quote(MDB2_Date::unix2Mdbstamp($this->currentLogin), $this->authTableCols['lastlogin']['type']) . '
                     WHERE '.$this->authTableCols['user_id']['name']
                        .'='.$this->dbc->quote($this->authUserId, $this->authTableCols['user_id']['type']);
    ...
    function readUserData($handle, $passwd=false)
    {
        ...
        if ($passwd !== false) {
            ...
            $sql    = 'SELECT '.implode(',', $fields).'
                       FROM '.$this->authTable.'
                       WHERE '.$this->authTableCols['handle']['name'].'='.$this->dbc->quote($handle, $this->authTableCols['handle']['type']).'
                        AND '.$this->authTableCols['passwd']['name'].'='.$this->dbc->quote($this->encryptPW($passwd), $this->authTableCols['passwd']['type']);
        } else {
