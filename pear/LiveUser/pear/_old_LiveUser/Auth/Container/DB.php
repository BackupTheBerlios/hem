 ...
 * @version $Id: DB.php,v 1.1 2004/07/16 13:58:45 mloitzl Exp $
 * @package LiveUser
 * @category authentication
 */
class LiveUser_Auth_Container_DB extends LiveUser_Auth_Common
{
    /**
     * dsn to connect to
     * @var object
     ...
     * @param  mixed
     */
    function &LiveUser_Auth_Container_DB(&$connectOptions)
    {
    ...
    /**
     * LiveUser_Auth_Container_DB::updateUserData()
     *
     ...
     *
     * @return mixed true or an DB error object
     * @access private
     */
    function updateUserData()
    {
        ...
        if (isset($this->authTableCols['lastlogin'])) {
            $sql  = 'UPDATE ' . $this->authTable.'
                     SET '    . $this->authTableCols['lastlogin'] . '=' . $this->dbc->quoteSmart(date('Y-m-d H:i:s', $this->currentLogin)) . '
                     WHERE '  . $this->authTableCols['user_id']   . '=' . $this->dbc->quoteSmart($this->authUserId);
    ...
    function readUserData($handle, $passwd = false)
    {
        ...
        if ($passwd !== false) {
            ...
            $sql    = 'SELECT '.implode(',', $fields).'
                       FROM   ' . $this->authTable.'
                       WHERE  ' . $this->authTableCols['handle'] . '=' . $this->dbc->quoteSmart($handle) . '
                        AND   ' . $this->authTableCols['passwd'] . '=' . $this->dbc->quoteSmart($this->encryptPW($passwd));
        } else {
        ...
        if (!DB::isError($result) && is_array($result)) {
            ...
            $this->authUserId   = $udata['user_id'];
            $this->lastLogin    = !empty($udata['lastlogin']) ?
                                    strtotime($udata['lastlogin']):'';
    ...
    function userExists($checkHandle = false,$checkPW = false)
    {
        ...
        if ($checkHandle !== false && $checkPW === false) {
            ...
            $sql    = 'SELECT ' . $this->authTableCols['user_id'] . '
                       FROM   ' . $this->authTable . '
                       WHERE  ' . $this->authTableCols['handle'] . '=' . $this->dbc->quoteSmart($checkHandle);
        } elseif ($checkHandle === false && $checkPW !== false) {
            // only search for the first user with the given password
            $sql    = 'SELECT ' . $this->authTableCols['user_id'] . '
                       FROM   ' . $this->authTable . '
                       WHERE  ' . $this->authTableCols['passwd'] . '=' . $this->dbc->quoteSmart($this->encryptPW($checkPW));
        } else {
            // check for a user with both handle and password matching
            $sql    = 'SELECT ' . $this->authTableCols['user_id'].'
                        FROM  ' . $this->authTable.'
                        WHERE ' . $this->authTableCols['handle'] . '=' . $this->dbc->quoteSmart($checkHandle) . '
                        AND   ' . $this->authTableCols['passwd'] . '=' . $this->dbc->quoteSmart($this->encryptPW($checkPW));
        }
